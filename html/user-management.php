...
include __DIR__ . '/auth.php';    // check user login 
    ...
    <div class="container-fluid flex-fill position-relative d-flex p-0">
        ...
        <div class="content w-100">
            ...
            <div class="container-fluid pt-0 px-0">
                <div class="page-header d-flex align-items-center justify-content-center">
                    <div class="text-center">
                        <h2 class="mb-1 text-white fw-bold"><i class="fas fa-users-cog me-2"></i>จัดการผู้ใช้งาน</h2>
                        <p class="mb-0 opacity-75">บริหารจัดการสิทธิ์และข้อมูลผู้ใช้งานในระบบ</p>
                    </div>
                </div>
            </div>
            ...
            <div class="container-fluid px-4 pb-5">
                <div class="card-box">
                    <div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-3">
                        <h5 class="fw-bold mb-0 text-dark"><i class="fas fa-list-ul me-2 text-primary"></i>รายชื่อผู้ใช้งาน</h5>
                    ...
                    <div class="table-responsive">
                        <table class="table table-custom table-hover table-bordered align-middle">
                            ...
                            <tbody>
                                <?php 
                                    // SQL Select all user --> user table in SQL database
                                    $users = include __DIR__ . '/db/db-user-select.php';
                                    if (!empty($users)): 
                                        $i=1;
                                ...
                                ?>
                                    <tr>  
                                        ...
                                        <td class="fw-bold text-primary"><?= htmlspecialchars($user['user']) ?></span></td>
                                        <td class="fw-bold text-primary"><?= htmlspecialchars($user['name']) ?></td>
                                        <td><?= htmlspecialchars($user['workgroup_name']) ?></td>
                                        <td><?= htmlspecialchars($user['department_name']) ?></td>
                                        <td class="text-center"><span class="badge bg-secondary"><?= htmlspecialchars($user['role_name']) ?></span></td>

                                        <td class="text-center">
                                            <button class="btn btn-navy btn-sm me-1 shadow-sm" data-bs-toggle="modal"
                                                data-bs-target="#editUserModal" data-id="<?= htmlspecialchars($user['id']) ?>"
                                                data-username="<?= htmlspecialchars($user['user']) ?>"
                                                data-name="<?= htmlspecialchars($user['name']) ?>"
                                                data-wg="<?= htmlspecialchars($user['workgroup_id']) ?>"
                                                data-dept="<?= htmlspecialchars($user['department_id']) ?>"
                                                data-role="<?= htmlspecialchars($user['role']) ?>">
                                                <i class="fas fa-edit"></i> แก้ไข
                                            </button>

                                            <a href="db/db-user-delete.php?id=<?= urlencode($user['id']) ?>"
                                                class="btn btn-danger btn-sm shadow-sm"
                                                onclick="return confirm('ยืนยันการลบ <?= addslashes($user['name']) ?> ?');">
                                ...
                                ?>
                                    <tr>
                                        <td colspan="7" class="text-center py-4 text-muted">ไม่มีข้อมูลผู้ใช้งาน</td>
                                    </tr>
                                <?php 
                                    endif; 
                                ?>
            ...
            <div class="modal fade" id="addUserModal" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content border-0 shadow">
                        <form method="POST" action="db/db-user-insert.php">
                            <div class="modal-header modal-header-custom">
                                <h5 class="modal-title fw-bold"><i class="fas fa-user-plus me-2"></i>เพิ่มผู้ใช้งาน</h5>
            ...
            <div class="modal fade" id="editUserModal" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content border-0 shadow">
                        <form method="POST" action="db/db-user-update.php">
                            <div class="modal-header modal-header-custom">
                                <h5 class="modal-title fw-bold"><i class="fas fa-user-edit me-2"></i>แก้ไขข้อมูลผู้ใช้งาน</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                            </div>
                            ...
                            <div class="modal-body p-4">
                                ...
                                <div class="mb-3">
                                    <label class="form-label fw-bold">ชื่อ-นามสกุล</label>
                                    <input type="text" name="fullname" id="editUserName" class="form-control" required>
                                </div>

                                <div class="mb-3">
                                    <label class="form-label fw-bold">รหัสผ่านใหม่ <span class="text-muted fw-normal small">(เว้นว่างถ้าไม่เปลี่ยน)</span></label>
                                    <input type="password" name="password" id="editUserPassword" class="form-control" placeholder="******">
                                </div>
